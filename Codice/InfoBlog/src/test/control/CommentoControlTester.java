...
public class CommentoControlTester extends Mockito
{
...
//	            System.out.println("get attribute value for key="+key+" : "+value);
	...
	@Test
    public void CommentoErrorTest() throws Exception 
	{   
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("Utente")).thenReturn("testutente@test.com");
        when(request.getParameter("commento")).thenReturn("");
        when(request.getRequestDispatcher("SingleArticle.jsp")).thenReturn(dispatcher);
	    new CommentoControl().doPost(request, response);
	    assertEquals("contenuto mancante",request.getAttribute("errore"));
	}
	...
	{   
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("Utente")).thenReturn("testutente@test.com");
        when(request.getParameter("commento")).thenReturn("commentoooo");
	...
	@Test
    public void NessunaModificaCommentoTest() throws Exception 
	{   
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("Utente")).thenReturn("testutente@test.com");
        when(request.getParameter("commento")).thenReturn("ottimo articolo");
	...
	@Test
    public void ModificaCommentoTest() throws Exception 
	{   
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("Utente")).thenReturn("testutente@test.com");
        when(request.getParameter("commento")).thenReturn("commentoooo");
        ...
        when(request.getRequestDispatcher("ViewArticleServlet?id=1&Titolo=ArticoloTest")).thenReturn(dispatcher);
	    new CommentoControl().doPost(request, response);
	    assertEquals("commento modificato",request.getAttribute("successo"));
	}
	
	@Test
    public void NuovoCommentoTest() throws Exception 
	{   
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("Utente")).thenReturn("testutente2@test.com");
        when(request.getParameter("commento")).thenReturn("commentoooo a questo articolo");
        when(request.getParameter("idArticolo")).thenReturn("1");
        when(request.getRequestDispatcher("ViewArticleServlet?id=1&Titolo=ArticoloTest")).thenReturn(dispatcher);
	    new CommentoControl().doPost(request, response);
	    assertEquals("commento pubblicato",request.getAttribute("successo"));
	}
